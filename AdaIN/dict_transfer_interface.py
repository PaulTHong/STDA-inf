...
def get_path_dict(root_dir):
    img_dict = {}
    class_dir = os.listdir(root_dir)
    for cls in class_dir:
        pre_path = os.path.join(root_dir, cls)
        test_list = os.listdir(pre_path)
        test_list = [os.path.join(pre_path, i) for i in test_list]
        img_dict[cls] = test_list
    return img_dict
...
def dict_style_transfer(img, class_name, style_path, vgg, decoder, content_size, style_size, dataset=None):
    '''
    img: content image, type: numpy.array()
    class_name: class of img
    style_path: path dict of style images
    '''
    ...
    style_list = style_path[class_name]
    S = len(style_list)
