...
if [ $1 = "train" ]; then
  ...
  STL10)
    if [ $train_mode = "baseline" ]; then
        #srun --job-name=stl_train -p GPU36 --gres=gpu:2 --qos low --time 120:00:00 \
            #python -u train.py --ckpt_name STL10_baseline.pth --optim Adam \
        ...
        srun --job-name=stl_train -p GPU36 --gres=gpu:2 --qos low --time 120:00:00 \
            python -u train.py --ckpt_name STL10_avgpool3_baseline.pth --optim Adam \
            --train_batch 256 --test_batch 256 --tra_augment --no_style_aug --multi_gpu --train_num_workers 4 \
            2>&1 |tee log/STL10_avgpool3_baseline_train.log
        ...
        #srun --job-name=stl_train -p GPU36 --gres=gpu:2 --qos low --time 120:00:00 \
            #python -u train.py --dataset STL10 \
            #--train_data_path ./data/STL10-data/train --test_data_path ./data/STL10-data/test \
  ...
  CALTECH256)
    if [ $train_mode = "baseline" ]; then
        srun --job-name=caltech_train -p GPU36 --gres=gpu:2 --qos low --time 120:00:00 \
            python -u train.py --dataset CALTECH256 \
        ...
        srun --job-name=caltech_train -p GPU36 --gres=gpu:2 --qos low --time 120:00:00 \
            python -u train.py --dataset CALTECH256 \
            --train_data_path ./data/CALTECH256/train --test_data_path ./data/CALTECH256/test \
            --ckpt_name CALTECH256_in_rand_list_100_styleaug.pth --lr 0.01 --milestones 60 120 --gamma 0.2 --max_epoch 200 --resume \
  ...
  CIFAR10)
    if [ $train_mode = "baseline" ]; then
      ...
      --tra_augment --no_style_aug 2>&1 |tee log/CIFAR10_baseline_train.log
        ...
        srun --job-name=cifar_train -p GPU36 --gres=gpu:1 --qos low --time 120:00:00 \
  ...
  STL10)
    for ((i=1; i<=1; i++)); do
        srun --job-name=stl_test -p GPU36 --gres=gpu:1 --qos low --time 120:00:00 \
        python -u fusion_test.py --base_test_path ./data/STL10-data/test \
        --include_base_test --style_test_mode list --style_path ./data/STL10-data/stl_random_style_list_per10 \
        --ckpt_name old_best_STL10_in_rand_list_per10_styleaug.pth --transform_round $i 
    ...
    for ((i=2; i<=15; i++)); do
        ...
        python -u fusion_test.py --base_test_path ./data/STL10-data/test \
        --include_base_test --style_test_mode list --style_path ./data/STL10-data/stl_random_style_list_per10 \
        --ckpt_name old_best_STL10_in_rand_list_per10_styleaug.pth --transform_round $i --load_style_test
